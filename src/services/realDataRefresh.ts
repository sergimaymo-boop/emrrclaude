  ...
  return {
    ...
    technical: {
      ...
      universeStats: {
        ...
        universeEligibleForScore: response.isGlobalTop8Final ? (response.assets ?? response.topCandidates)?.length ?? 0 : 0,
        universeRanked: isGlobal ? (response.assets ?? response.topCandidates)?.length ?? 0 : 0,
        finalTop8Count: isGlobal ? (response.assets ?? response.topCandidates)?.length ?? 0 : 0,
