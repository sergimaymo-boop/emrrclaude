...
| Frontend | `src/services/rallyTestRefresh.ts` (fórmula/cifras en `RALLY_TEST_BASELINE`; `estimateNextReview` propio a +61 días) · `src/components/RallyTestPanel.tsx` (violeta `#a855f7`; el pie publica el acta del auditor, incluidas las cifras de riesgo real) |
...
- **🔎 AUDITORÍA COMPLETA 25-sep-2026 — Fases 1 y 2 aplicadas (commits `26b278c`, `1d47ca1`)**. Norma de Sergi: *datos reales siempre; con mercado cerrado, el último cierre rotulado; si un dato no se puede actualizar, SE DICE*. Contratos nuevos que no hay que romper:
  - `providerCascade._parseYahooBars` devuelve `{ bars, gapDates, lastBarForming }`: `lastBarForming` = la última vela es de una sesión AÚN ABIERTA (precio intradía, nunca "cierre"), según `meta.currentTradingPeriod` de Yahoo. Los motores Rally/Rally-Test emiten `metrics.lastBarDate / lastBarForming / missingSessions` y los paneles los pintan ("sesión 25-sep · EN CURSO"). ⚠ Si TwelveData gana la carrera de histórico (`raceProviders`, clave configurada en Vercel) su serie NO trae esas marcas — pendiente decidir si Yahoo pasa a ser primario.
