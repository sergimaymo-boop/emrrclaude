...
// ─── RONDA 5: finalistas robustos, medidos en las DOS mitades ────────────────
...
const W_RSML= { rs: 0.45, mom: 0.45, trend: 0, prox: 0, rvol: 0, atr: 0, liq: 0.10 };

const FINALISTS = [];
for (const [wn, W] of [["momento", W_MOM], ["RS+momento", W_RSM], ["RS+mom+liquidez", W_RSML], ["v2.0 actual", BASE_W]])
  for (const review of [63, 84, 126])
    for (const noPenalties of [true, false])
      FINALISTS.push({ wn, W, review, topN: 10, noPenalties,
        label: `${wn} · rev${review} · top10 ${noPenalties ? "sin pen." : "con pen."}` });

console.log("Finalistas a 10 tickers, medidos por separado en las dos mitades\n");
console.log("Configuración".padEnd(42), "1ª mitad".padStart(9), "2ª mitad".padStart(9), "PEOR".padStart(7), "  caídas 1ª/2ª", "  Sharpe peor");
const scored = FINALISTS.map((c) => {
  const a = simulate({ ...c, from: 260, to: SPLIT });
  const b = simulate({ ...c, from: SPLIT, to: D - 1 });
  return { ...c, a, b, worst: Math.min(a.cagr, b.cagr), worstSharpe: Math.min(a.sharpe, b.sharpe), worstMdd: Math.max(a.mdd, b.mdd) };
});
// El criterio de elección es el PEOR de los dos periodos: robustez, no suerte.
scored.sort((x, y) => y.worst - x.worst);
for (const s of scored) {
  console.log(s.label.padEnd(42), f1(s.a.cagr).padStart(9), f1(s.b.cagr).padStart(9), f1(s.worst).padStart(7),
    `  ${f1(s.a.mdd, 0)}/${f1(s.b.mdd, 0)}`.padEnd(16), s.worstSharpe.toFixed(2).padStart(6));
}
const win = scored[0];
console.log(`\nGANADOR POR ROBUSTEZ: ${win.label}`);
const full = simulate({ ...win, from: 260, to: D - 1 });
console.log(`  Periodo completo 2017-2026: CAGR ${f1(full.cagr)} · caída máxima ${f1(full.mdd)} · MAR ${full.mar.toFixed(2)} · Sharpe ${full.sharpe.toFixed(2)} · ${full.tradesYr.toFixed(0)} operaciones/año`);
console.log(`  Tercios: ${full.sub.map((s) => f1(s, 0)).join(" / ")}`);
const bhFull = buyHoldSpy(260, D - 1);
console.log(`  SPY comprar y mantener:     CAGR ${f1(bhFull.cagr)} · caída máxima ${f1(bhFull.mdd)} · MAR ${bhFull.mar.toFixed(2)}`);

// Selección ACTUAL con la fórmula ganadora: los 10 de hoy.
const last = D - 1;
const cands = [];
for (let ti = 0; ti < T.length; ti++) {
  const t = T[ti], f = t.feat[last];
  if (!f || !isNum(t.adj[last])) continue;
  const s = scoreOf(f, t.region, win.W, { noPenalties: win.noPenalties });
  if (s != null) cands.push({ sym: t.sym, name: t.name, s, m3: f.m3, m6: f.m6, rs3m: f.rs3m, prox: f.prox, atr: f.atr });
}
cands.sort((a, b) => b.s - a.s);
console.log(`\n=== LOS 10 DE HOY (${dates[last]}) con la fórmula ganadora ===`);
cands.slice(0, 10).forEach((c, i) => console.log(
  `  ${String(i + 1).padStart(2)}. ${c.sym.padEnd(12)} ${String(c.name).slice(0, 26).padEnd(27)} puntuación ${c.s.toFixed(1).padStart(5)}  ·  3m ${f1((c.m3 ?? 0) / 100, 0).padStart(6)}  6m ${f1((c.m6 ?? 0) / 100, 0).padStart(6)}  vs SP500 ${f1((c.rs3m ?? 0) / 100, 0).padStart(6)}`));

fs.writeFileSync("backtests/rally-backtest-5-final.json", JSON.stringify({ ranAt: new Date().toISOString(),
  winner: { weights: win.W, review: win.review, topN: win.topN, noPenalties: win.noPenalties, label: win.label },
  full, buyHold: bhFull, halves: scored.map(({ W, a, b, ...s }) => ({ ...s, aCagr: a.cagr, bCagr: b.cagr })),
  todayTop10: cands.slice(0, 10) }, null, 1));
console.log("\nGuardado: backtests/rally-backtest-5-final.json");
