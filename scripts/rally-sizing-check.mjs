...
function portfolio({ topN = 10, review = 84, poolN = null, runwayMode = null, exhaust = "off",
  ...
  for (let i = FROM + 1; i <= TO; i++) {
    ...
    const newIds = pick.map((p) => ({ ti: p.ti, w: weighting === "score" ? Math.max(0.1, p.rs - 50) : 1 }));
...
function ev3(cfg) {
  ...
  return { label: cfg.label, w63: a.worst, w84: b.worst, w105: c.worst, minAll: Math.min(a.worst, b.worst, c.worst),
           cagr: b.cagr, mdd: b.mdd, mar: b.mar };
}
console.log("Config".padEnd(34), "rev63".padStart(8), "rev84".padStart(8), "rev105".padStart(8), "PEOR-3".padStart(8), " ‖ ", "CAGR84".padStart(8), "caída".padStart(8), "MAR".padStart(6));
const tests = [
  { label: "BASE: top10, peso igual", topN: 10, weighting: "equal" },
  { label: "top10, peso por convicción", topN: 10, weighting: "score" },
  { label: "top5, peso igual", topN: 5, weighting: "equal" },
  { label: "top8, peso igual", topN: 8, weighting: "equal" },
  { label: "top12, peso igual", topN: 12, weighting: "equal" },
  { label: "top15, peso igual", topN: 15, weighting: "equal" },
  { label: "top20, peso igual", topN: 20, weighting: "equal" },
];
const out = tests.map((t) => { const r = ev3(t);
  console.log(r.label.padEnd(34), f1(r.w63).padStart(8), f1(r.w84).padStart(8), f1(r.w105).padStart(8), f1(r.minAll).padStart(8), " ‖ ",
    f1(r.cagr).padStart(8), f1(r.mdd).padStart(8), r.mar.toFixed(2).padStart(6));
  return r; });
console.log("\n=== Por PEOR de las 3 cadencias (criterio duro) ===");
[...out].sort((a,b)=>b.minAll-a.minAll).forEach((o)=>console.log("  " + o.label.padEnd(34), f1(o.minAll).padStart(8), "· CAGR", f1(o.cagr).padStart(7), "· caída", f1(o.mdd).padStart(7), "· MAR", o.mar.toFixed(2)));
fs.writeFileSync("backtests/rally-sizing-topn.json", JSON.stringify({ ranAt: new Date().toISOString(), out }, null, 1));
console.log("\nGuardado: backtests/rally-sizing-topn.json");
