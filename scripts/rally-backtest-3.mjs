...
function simulate({ topN = 10, review = 21, W = BASE_W, regimeFilter = false, trailPct = null,
                    minScore = 0, noPenalties = false, label = "", from = 260, to = null }) {
...
// ─── RONDA 3: ¿aguanta fuera de muestra? ─────────────────────────────────────
const f1 = (x, d = 1) => `${(x * 100).toFixed(d)}%`;
const SPLIT = Math.floor((D - 260) / 2) + 260;   // mitad del periodo
console.log(`Entrenamiento: ${dates[260]} → ${dates[SPLIT]}   |   Prueba ciega: ${dates[SPLIT]} → ${dates.at(-1)}\n`);
  ...
  for (const review of [42, 63, 84, 126])
    for (const topN of [5, 10, 15])
      for (const noPenalties of [false, true])
        combos.push({ wn, W, review, topN, noPenalties, label: `${wn}·rev${review}·top${topN}${noPenalties ? "·sinPen" : ""}` });

const rows = combos.map((c) => {
  const tr = simulate({ ...c, from: 260, to: SPLIT });
  const te = simulate({ ...c, from: SPLIT, to: D - 1 });
  return { label: c.label, trainCagr: tr.cagr, trainMar: tr.mar, testCagr: te.cagr, testMdd: te.mdd, testMar: te.mar, testSharpe: te.sharpe, testOps: te.tradesYr };
});
const bhTr = buyHoldSpy(260, SPLIT), bhTe = buyHoldSpy(SPLIT, D - 1);

const hdr = () => console.log("Variante".padEnd(34), "ENTREN.CAGR".padStart(12), "MAR".padStart(6), " ‖ ", "PRUEBA CAGR".padStart(12), "MaxDD".padStart(7), "MAR".padStart(6), "Sharpe".padStart(7));
const ln = (r) => console.log(r.label.padEnd(34), f1(r.trainCagr).padStart(12), r.trainMar.toFixed(2).padStart(6), " ‖ ",
  f1(r.testCagr).padStart(12), f1(r.testMdd).padStart(7), r.testMar.toFixed(2).padStart(6), r.testSharpe.toFixed(2).padStart(7));

console.log("=== REFERENCIA SPY ===");
console.log(`  entrenamiento CAGR ${f1(bhTr.cagr)} (MAR ${bhTr.mar.toFixed(2)})  ‖  prueba ciega CAGR ${f1(bhTe.cagr)}  MaxDD ${f1(bhTe.mdd)}  MAR ${bhTe.mar.toFixed(2)}  Sharpe ${bhTe.sharpe.toFixed(2)}`);

console.log("\n=== Los 10 mejores DEL ENTRENAMIENTO, y qué hicieron después a ciegas ===");
hdr();
[...rows].sort((a, b) => b.trainMar - a.trainMar).slice(0, 10).forEach(ln);

console.log("\n=== Los 10 mejores de la PRUEBA CIEGA (para ver el techo real) ===");
hdr();
[...rows].sort((a, b) => b.testMar - a.testMar).slice(0, 10).forEach(ln);

// ¿El ranking del entrenamiento predice el de la prueba? (correlación de Spearman)
const rk = (arr, key) => { const s = [...arr].sort((a, b) => b[key] - a[key]); const m = new Map(); s.forEach((r, i) => m.set(r.label, i + 1)); return m; };
const r1 = rk(rows, "trainMar"), r2 = rk(rows, "testMar");
const n = rows.length;
let d2 = 0; for (const r of rows) d2 += (r1.get(r.label) - r2.get(r.label)) ** 2;
const rho = 1 - (6 * d2) / (n * (n * n - 1));
console.log(`\nCorrelación entre "bueno en entrenamiento" y "bueno a ciegas": rho = ${rho.toFixed(2)}  (n=${n})`);
console.log(rho > 0.5 ? "  → La elección de parámetros SÍ se transfiere: elegir por backtest tiene sentido."
          : rho > 0.2 ? "  → Transferencia DÉBIL: el backtest orienta, pero no garantiza."
                      : "  → NO se transfiere: elegir el mejor del backtest sería sobreajuste puro.");

fs.writeFileSync("backtests/rally-backtest-3-oos.json", JSON.stringify({ ranAt: new Date().toISOString(),
  split: dates[SPLIT], bhTrain: bhTr, bhTest: bhTe, rho, rows }, null, 1));
console.log("\nGuardado: backtests/rally-backtest-3-oos.json");
