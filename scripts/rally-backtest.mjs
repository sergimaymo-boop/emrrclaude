...
// ─── modo --verify: equivalencia réplica ↔ motor de producción ───────────────
if (process.argv.includes("--verify")) {
  ...
  for (let ti = 0; ti < T.length; ti += 23) {
    const t = T[ti];
    const bars = series[t.sym].bars;
    for (const frac of [0.6, 0.8, 1.0]) {
      const cut = Math.floor(bars.length * frac);
      if (cut < 300) continue;
      const endDate = bars[cut - 1].d;
      const k = dateIdx.get(endDate);
      if (k === undefined || !t.feat[k]) continue;
  ...
  console.log(`\nEQUIVALENCIA: ${n} muestras · desvío máximo ${maxDiff} puntos · ${bad} por encima de 2 puntos`);
...
function head(t) {
  console.log(`\n=== ${t} ===`);
  console.log("Variante".padEnd(38), "CAGR".padStart(7), "MaxDD".padStart(7), "MAR".padStart(6), "Sharpe".padStart(7), "ops/a".padStart(6), "%dentro".padStart(8), "  tercios");
}
function line(r) {
  console.log(r.label.padEnd(38), f1(r.cagr).padStart(7), f1(r.mdd).padStart(7), r.mar.toFixed(2).padStart(6),
...
head("A) ¿Funciona el ranking tal y como está hoy en el código? (pesos v2.0 originales)");
line(bh);
for (const review of [5, 10, 21, 63]) {
  const r = simulate({ topN: 10, review, label: `RALLY v2.0 top10 · revisión ${review}d` });
  results.push(r); line(r);
}

head("B) ¿Cuántos tickers? (revisión mensual)");
for (const topN of [3, 5, 10, 15, 20]) {
  const r = simulate({ topN, review: 21, label: `RALLY v2.0 top${topN} · mensual` });
  results.push(r); line(r);
}

head("C) ¿Ayuda filtrar por régimen de mercado (SPY sobre su EMA200)?");
for (const topN of [5, 10]) for (const regimeFilter of [false, true]) {
  const r = simulate({ topN, review: 21, regimeFilter, label: `top${topN} · ${regimeFilter ? "CON" : "sin"} filtro de régimen` });
  results.push(r); line(r);
}

head("D) ¿Ayuda el trailing stop?");
for (const trailPct of [null, 0.08, 0.12, 0.15, 0.20]) {
  const r = simulate({ topN: 10, review: 21, regimeFilter: true, trailPct, label: `top10 · régimen · trailing ${trailPct ? f1(trailPct, 0) : "no"}` });
  results.push(r); line(r);
}

head("E) ¿Y si cambiamos los PESOS de la fórmula? (top10 · mensual · régimen)");
const WEIGHT_VARIANTS = {
  "v2.0 original":            BASE_W,
  "solo fuerza relativa":     { rs: 1.00, mom: 0, trend: 0, prox: 0, rvol: 0, atr: 0, liq: 0 },
  "solo momento":             { rs: 0, mom: 1.00, trend: 0, prox: 0, rvol: 0, atr: 0, liq: 0 },
  "solo tendencia":           { rs: 0, mom: 0, trend: 1.00, prox: 0, rvol: 0, atr: 0, liq: 0 },
  "RS+momento 50/50":         { rs: 0.50, mom: 0.50, trend: 0, prox: 0, rvol: 0, atr: 0, liq: 0 },
  "RS 50 · mom 30 · tend 20": { rs: 0.50, mom: 0.30, trend: 0.20, prox: 0, rvol: 0, atr: 0, liq: 0 },
  "sin volumen (RVOL a 0)":   { rs: 0.37, mom: 0.26, trend: 0.19, prox: 0.08, rvol: 0, atr: 0.05, liq: 0.05 },
  "más proximidad 52s":       { rs: 0.30, mom: 0.20, trend: 0.15, prox: 0.25, rvol: 0.05, atr: 0.03, liq: 0.02 },
};
for (const [name, W] of Object.entries(WEIGHT_VARIANTS)) {
  const r = simulate({ topN: 10, review: 21, regimeFilter: true, W, label: name });
  results.push(r); line(r);
}

head("F) ¿Las penalizaciones (parabólico, distribución…) suman o restan?");
for (const noPenalties of [false, true]) {
  const r = simulate({ topN: 10, review: 21, regimeFilter: true, noPenalties, label: `top10 · régimen · ${noPenalties ? "SIN" : "con"} penalizaciones` });
  results.push(r); line(r);
}

console.log("\nEjemplo de selección (primeras revisiones):");
for (const p of (results[0].picksLog ?? [])) console.log(`  ${p.date}: ${p.top.join(", ")}`);

fs.writeFileSync("backtests/rally-backtest-1.json", JSON.stringify({
  ranAt: new Date().toISOString(), period: [dates[260], dates.at(-1)], tickers: T.length,
  buyHold: bh, results: results.map(({ picksLog, ...r }) => r),
}, null, 1));
console.log("\nGuardado: backtests/rally-backtest-1.json");
