...
import { computeSp500Signal, buildSp500Order, SP500_CALIBRATION } from "../api/_lib/sp500Engine.js";
...
const COST = 5 / 1e4, FEE = 0.0009;
const EXPECTED = SP500_CALIBRATION.backtest.core;
...
for (const profile of ["EQUILIBRADO", "PRUDENTE", "AMBICIOSO", "AGRESIVO"]) {
  ...
  if (profile === "EQUILIBRADO") {
    const dC = Math.abs(cagr - EXPECTED.cagr) * 100, dD = Math.abs(mdd - EXPECTED.maxDD) * 100;
    console.log(`  ↳ esperado del estudio: CAGR ${(EXPECTED.cagr * 100).toFixed(1)}% / caída ${(EXPECTED.maxDD * 100).toFixed(1)}%  →  desvío ${dC.toFixed(1)} pp / ${dD.toFixed(1)} pp  ${dC < 1.5 && dD < 3 ? "✅ REPRODUCE" : "❌ NO REPRODUCE"}`);
  }
}
