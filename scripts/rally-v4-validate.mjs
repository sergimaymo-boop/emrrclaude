...
function portfolioV4({ review, FROM, weighting }) {
  ...
  for (let i = FROM + 1; i <= TO; i++) {
    ...
    for (let ti = 0; ti < T.length; ti++) {
      ...
      const s = scoreV4(f);
      if (s != null) cands.push({ ti, s });
    }
    cands.sort((a, b) => b.s - a.s);
    let pick = cands.slice(0, 10).map((c) => ({ ti: c.ti, w: 1 }));
    if (weighting === "conviction") {
      const raws = cands.slice(0, 10).map((c) => Math.max(0.1, c.s - 50));
      const tot = raws.reduce((x, y) => x + y, 0) || 1;
      pick = cands.slice(0, 10).map((c, j) => ({ ti: c.ti, w: clamp((raws[j] / tot) * 100, 4, 18) }));
    }
...
console.log("Config".padEnd(26), "9 celdas (peor semestre)".padEnd(72), "mín".padStart(7), "mediana".padStart(8));
const summary = {};
for (const weighting of ["equal", "conviction"]) {
  const cells = [];
  for (const FROM of PHASES) for (const review of REVIEWS) {
    cells.push(evalCurve(portfolioV4({ review, FROM, weighting }), FROM).worst);
  }
  const minC = Math.min(...cells), medC = [...cells].sort((a,b)=>a-b)[4];
  summary[weighting] = { cells, minC, medC };
  console.log(`mom9·${weighting}`.padEnd(26), cells.map((c) => f1(c).padStart(7)).join(" "), f1(minC).padStart(7), f1(medC).padStart(8));
}
let convWins = 0;
for (let i = 0; i < 9; i++) if (summary.conviction.cells[i] >= summary.equal.cells[i]) convWins++;
console.log(`\nConvicción iguala o mejora al peso igual en ${convWins}/9 celdas.`);

// cifras canónicas para publicar (fase 260 · revisión 84)
const canon = evalCurve(portfolioV4({ review: 84, FROM: 260, weighting: "conviction" }), 260);
const canonEq = evalCurve(portfolioV4({ review: 84, FROM: 260, weighting: "equal" }), 260);
console.log(`\nCANÓNICO v4 (fase 260·rev84): convicción CAGR ${f1(canon.cagr)} · caída ${f1(canon.mdd)} · MAR ${(canon.cagr/canon.mdd).toFixed(2)}`);
console.log(`                              peso igual  CAGR ${f1(canonEq.cagr)} · caída ${f1(canonEq.mdd)} · MAR ${(canonEq.cagr/canonEq.mdd).toFixed(2)}`);

// los 10 de HOY con la fórmula v4
const last = D - 1;
const today = [];
for (let ti = 0; ti < T.length; ti++) {
  const f = T[ti].feat[last];
  if (!f || !isNum(T[ti].adj[last])) continue;
  const s = scoreV4(f);
  if (s != null) today.push({ sym: T[ti].sym, s, m9: f.m9 });
}
today.sort((a, b) => b.s - a.s);
console.log("\nLOS 10 DE HOY con la fórmula v4 (momento 9m):");
today.slice(0, 10).forEach((t, i) => console.log(`  ${i + 1}. ${t.sym.padEnd(12)} score ${t.s.toFixed(1)}  ·  momento 9m ${t.m9.toFixed(0)}%`));

fs.writeFileSync("backtests/rally-v4-validation.json", JSON.stringify({ ranAt: new Date().toISOString(), summary, canon, canonEq, today: today.slice(0, 15) }, null, 1));
console.log("\nGuardado: backtests/rally-v4-validation.json");
