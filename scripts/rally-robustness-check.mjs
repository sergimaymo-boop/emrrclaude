...
function portfolio({ topN = 10, review = 84, poolN = null, runwayMode = null, exhaust = "off",
  ...
  for (let i = FROM + 1; i <= TO; i++) {
    let r = 0;
    if (held.length) { for (const ti of held) { const t = T[ti]; const a = t.adj[i], b = t.adj[i - 1];
    ...
    const newIds = pick.map((p) => p.ti);
    const oldSet = new Set(held), newSet = new Set(newIds);
    let turnover = 0;
...
const rows = [];
function run(cfg) { const r = evaluate(cfg); rows.push({ ...r, cfg }); return r; }

// ── ¿el hallazgo "pool → edad de la tendencia" aguanta al cambiar la cadencia? ──
console.log("PRUEBA DE ROBUSTEZ: si solo funciona a 84 sesiones, es ruido.\n");
console.log("Config".padEnd(30), "rev63 peor".padStart(11), "rev84 peor".padStart(11), "rev105 peor".padStart(12), " ‖ ", "rev84 CAGR".padStart(11), "caída".padStart(8), "MAR".padStart(6));
const configs = [
  { label: "BASE (sin recorrido)", poolN: null, runwayMode: null },
  { label: "pool 12 → edad", poolN: 12, runwayMode: "age" },
  { label: "pool 15 → edad", poolN: 15, runwayMode: "age" },
  { label: "pool 18 → edad", poolN: 18, runwayMode: "age" },
  { label: "pool 20 → edad", poolN: 20, runwayMode: "age" },
  { label: "pool 25 → edad", poolN: 25, runwayMode: "age" },
];
const out = [];
for (const c of configs) {
  const r63 = evaluate({ ...c, review: 63 });
  const r84 = evaluate({ ...c, review: 84 });
  const r105 = evaluate({ ...c, review: 105 });
  out.push({ ...c, w63: r63.worst, w84: r84.worst, w105: r105.worst, cagr84: r84.cagr, mdd84: r84.mdd, mar84: r84.mar,
             minAll: Math.min(r63.worst, r84.worst, r105.worst) });
  console.log(c.label.padEnd(30), f1(r63.worst).padStart(11), f1(r84.worst).padStart(11), f1(r105.worst).padStart(12), " ‖ ",
    f1(r84.cagr).padStart(11), f1(r84.mdd).padStart(8), r84.mar.toFixed(2).padStart(6));
}
const base = out[0];
console.log("\n=== ¿Alguna configuración mejora a la BASE en las TRES cadencias? ===");
const winners = out.slice(1).filter((o) => o.w63 >= base.w63 && o.w84 >= base.w84 && o.w105 >= base.w105);
if (!winners.length) {
  console.log("  NINGUNA. El hallazgo NO es robusto: solo aparecía a 84 sesiones → era ruido de selección.");
} else {
  for (const w of winners) console.log(`  ${w.label}: mejora en las 3 cadencias (peor global ${f1(w.minAll)} frente a ${f1(base.minAll)} de la base)`);
}
console.log("\n=== Peor resultado en cualquiera de las 3 cadencias (criterio más duro) ===");
[...out].sort((a,b)=>b.minAll-a.minAll).forEach((o)=>console.log("  " + o.label.padEnd(30), f1(o.minAll).padStart(8)));

fs.writeFileSync("backtests/rally-robustness-cadence.json", JSON.stringify({ ranAt: new Date().toISOString(), out }, null, 1));
console.log("\nGuardado: backtests/rally-robustness-cadence.json");
