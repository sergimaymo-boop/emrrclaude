...
const rows = [];
function run(cfg) { const r = evaluate(cfg); rows.push(r);
  console.log(r.label.padEnd(44), f1(r.cagr).padStart(7), f1(r.mdd).padStart(7), r.mar.toFixed(2).padStart(6),
    f1(r.cagrA).padStart(9), f1(r.cagrB).padStart(9), f1(r.worst).padStart(8)); }

console.log("Variante".padEnd(44), "CAGR".padStart(7), "MaxDD".padStart(7), "MAR".padStart(6), "1ª mitad".padStart(9), "2ª mitad".padStart(9), "PEOR".padStart(8));
console.log("\n--- BASE actual ---");
run({ label: "v3.0 base (top10 por momento)" });

console.log("\n--- DOS ETAPAS: mejores por momento → los de más recorrido ---");
for (const poolN of [15, 20, 30, 50]) {
  for (const mode of ["compuesto", "age", "ext", "completion"]) {
    run({ poolN, runwayMode: mode, label: `pool ${poolN} → recorrido "${mode}"` });
  }
}

console.log("\n--- EXCLUSIÓN suave de los agotados (sin reordenar el resto) ---");
for (const exhaust of ["suave", "medio", "duro"]) run({ exhaust, label: `excluir agotados: ${exhaust}` });

console.log("\n--- EXCLUSIÓN + dos etapas ---");
for (const exhaust of ["suave", "medio"]) for (const poolN of [20, 30]) {
  run({ exhaust, poolN, runwayMode: "compuesto", label: `excluir ${exhaust} + pool ${poolN}` });
}

console.log("\n=== RANKING POR ROBUSTEZ (peor de las dos mitades) ===");
[...rows].sort((a, b) => b.worst - a.worst).slice(0, 12).forEach((r) =>
  console.log(r.label.padEnd(44), "peor", f1(r.worst).padStart(7), "· completo", f1(r.cagr).padStart(7), "· caída", f1(r.mdd).padStart(7), "· MAR", r.mar.toFixed(2)));

fs.writeFileSync("backtests/rally-optimize-selection.json", JSON.stringify({ ranAt: new Date().toISOString(),
  rows: rows.map(({ curve, ...r }) => r) }, null, 1));
console.log("\nGuardado: backtests/rally-optimize-selection.json");
