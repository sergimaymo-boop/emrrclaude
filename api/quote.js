...
function parseAllowedSymbol(rawSymbol) {
  if (Array.isArray(rawSymbol) || typeof rawSymbol !== "string") return null;
  const normalized = rawSymbol.trim().toUpperCase();
  return ALLOWED_SYMBOLS.includes(normalized) ? normalized : null;
}

function hasMultiSymbolInput(rawSymbol) {
  return Array.isArray(rawSymbol) || (typeof rawSymbol === "string" && rawSymbol.includes(","));
}
...
export default async function handler(request, response) {
  if (request.method && request.method !== "GET") {
    return sendJson(response, 405, {
      ...
      message: "Only GET is allowed for Phase 5 controlled quote validation.",
    });
  }

  const rawSymbol = request.query?.symbol;

  if (hasMultiSymbolInput(rawSymbol)) {
    return sendJson(response, 400, {
      ok: false,
      error: "MULTI_SYMBOL_BLOCKED",
      message: "Phase 5 allows only one symbol per request.",
      allowedSymbols: ALLOWED_SYMBOLS,
    });
  }

  const symbol = parseAllowedSymbol(rawSymbol);

  if (!symbol) {
    return sendJson(response, 400, {
      ok: false,
      error: "SYMBOL_NOT_ALLOWED",
      message: "Symbol is missing or not in the Phase 5 allowlist.",
      allowedSymbols: ALLOWED_SYMBOLS,
    });
  }

  const quote = await getControlledQuote(symbol);
